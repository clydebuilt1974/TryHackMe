...
* Put simply, the “S” in SSH stands for secure, which can be summarised simply as:
...
* On Linux, macOS, and MS Windows builds after 2018, you can connect to an SSH server using the following command ssh username@10.10.158.132.
* This command will try to connect to the server of IP address 10.10.158.132 with the login name username.
* If an SSH server is listening on the default port, it will ask you to provide the password for username.
* Once authenticated, the user will have access to the target server’s terminal.
* The terminal output below is an example of using SSH to access a Debian Linux server.
```
...
mark@debian8:~$
```
* In the example above, we issued the command ssh mark@10.10.158.132.
...
* As explained earlier, MITM takes place when a malicious party, E, situates itself between A and B, and communicates with A, pretending to be B, and communicates with B pretending to be A, while A and B think that they are communicating directly with each other.
...
* This attack is shown in the image below.
* We can use SSH to transfer files using SCP (Secure Copy Protocol) based on the SSH protocol.
* An example of the syntax is as follows: scp mark@10.10.158.132:/home/mark/archive.tar.gz ~.
...
* Another example syntax is scp backup.tar.bz2 mark@10.10.158.132:/home/mark/.
...
* The third type of attack that we will cover is a password attack.
* Many protocols require you to authenticate.
...
* In the POP3 example below, we are identified as the user frank, and the server authenticated us because we provided the correct password.
* In other words, the password is one way to authenticate.
```
...
> 123456, 123456789, password, adobe123, 12345678, qwerty, 1234567, 111111, photoshop, 123123
...
* Others haven’t realised yet that qwerty is not secret, and it is used by many as their password.
...
* Over time, hackers have compiled list after list containing leaked passwords from data breaches.
...
* Mitigation against such attacks can be sophisticated and depends on the target system.
...
* It works well if the login page is via a graphical user interface (GUI).
...
* This approach works well with SSH, for instance.
